...
alpha=0.35  			#EXPONENTIAL DECAY FACTOR FOR MOMENTUM ALGO
algo='MOM'
...
model_type="ANN";  LR=0.1  #LEARNING RATE 
#ANN PARAM
...
def NN_eval(x,sub):
	if activation=="TANH":
		...
		xtemp = np.transpose(x)
		for i in range(0,int(len(sub)*(1/2))-1):
			mul = np.matmul(sub[2*i],xtemp) + sub[2*i+1]
			act = 2*(S(2*mul)-0.5)
			xtemp = act
		f = np.matmul(sub[-2],xtemp) + sub[-1]
		final = np.transpose(f)
		return final
...
def loss(p,index_2_use):
	...
	regu = sum(errors**2.0) + GAMMA_L1*sum(p) + GAMMA_L2*sum(p**2)
	training_loss=regu/len(index_2_use)				#MSE with L1/L2 regularization
